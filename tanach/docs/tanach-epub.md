...
The reader also fingerprints each recognized Tanach EPUB and imports its structured content into a disposable per-book SQLite database. Chapter XHTML is gzip-compressed, search uses normalized substring matching (`instr` on a Hebrew-mark-normalized text column) with visibility filters, chapters are projected lazily for the selected translation/commentaries, and at most three projected Tanach chapters remain resident. EPUB remains canonical; `library.json` continues to own positions, settings, bookmarks, and annotations. All 39 full EPUBs passed the final-schema host import, warm-reopen, first/last chapter load, and substring-search integration test on 18 September 2026 against the then-current build. The current 22 September rebuild has not yet been run through the env-gated integration test suite; those tests require TANACH_FULL_BOOKS_DIR and TANACH_SAMPLES_DIR environment variables. The run took 5:01 and produced 486,289,408 bytes (463.8 MiB) of cache databases in total before cleanup, below the 768 MiB cache ceiling. These checks do not replace physical Bigme performance validation.

> **TODO:** The docs previously described the search as FTS5-based. The actual implementation uses `instr()` on a normalized `search_text` TEXT column. This gap between documentation and implementation should be resolved — either implement FTS5 (with proper Hebrew tokenization and word-boundary preservation) or keep the `instr()` approach and ensure all docs consistently describe it as normalized substring matching. Related: Hebrew vowel preservation in search is not currently supported (all vowels are stripped before indexing).
...
> **TODO:** Chapter XHTML compression — verify actual compression ratios and decompression cost. The reader-compatibility.md notes Genesis 1 XHTML is ~3 MB uncompressed. The pipeline gzip-compresses chapter XHTML into the SQLite cache, but the normalized `search_text` column is stored uncompressed (duplicating all text). Measure actual per-chapter compression ratios, decompression overhead on `loadChapter`, and whether higher gzip levels or alternative compression would reduce the 463.8 MiB total cache size meaningfully.

> **TODO:** Projection key and settings-dependent caching. The projection key includes `commentarySources` (sorted), `commentaryLanguage`, `studyTranslation`, and `honorPublisherCss`. Rapid switching between different commentary source combinations causes re-projection. Evaluate whether the 3-chapter resident limit is sufficient under heavy switching, and whether certain common combinations should be proactively cached.
...
> **TODO:** Cache key stability and portability. The JSON parse cache key is `sha256('$version:${doc.id}:$fingerprint:$honorPublisherCss')`. The SQLite database path is `sha256(doc.id)`. The doc ID is derived from the first 64 KiB of the file plus file size — two books with identical first 64 KiB and file size would collide (unlikely in practice for Tanach). The caches are disposable and device-local; exporting/importing the 463+ MB cache to another device is not supported, so new devices must re-import. Cache directory location (`getApplicationCacheDirectory()`) may be cleared by Android under storage pressure.
